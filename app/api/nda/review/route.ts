...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { report, row } = await reviewNdaDocument(buffer, file.name, file.type);
    return apiOk("NDA review complete", { report, row });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    console.error("[api/nda/review]", file.name, message);
